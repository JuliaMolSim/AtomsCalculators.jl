...
- [`force_unit`](@ref) : compute force unit from energy and length units 
- [`zero_energy`](@ref) : initialize potential energy 
- [`zero_forces`](@ref) : initilize a force vector 
- [`zero_virial`](@ref) : initialize a virial matrix 

A calculator may provide non-allocating and or combined calculations that can sometimes be preferred for performance reasons. These can also be generated by the [`@generate_interface`](@ref) macro. 

- [`energy_forces`](@ref) for potential energy and allocating forces
- [`energy_forces!`](@ref) for potential energy and non-allocating forces
- [`energy_forces_virial`](@ref) for potential energy, allocating forces and virial
- [`energy_forces_virial!`](@ref) for potential energy, non-allocating forces and virial
